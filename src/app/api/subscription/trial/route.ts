      ...
      if (pixelId && accessToken) {
        ...
        await sendMetaPurchaseEvent({
          ...
          eventSourceUrl: storeData?.customDomain
            ? `https://${storeData.customDomain}/order/confirmation?orderId=${orderId}`
            : `https://${storeData?.subdomain}.sellquic.com/order/confirmation?orderId=${orderId}`,
        });
...
existingPeriodsSnap.docs.forEach(doc => {
  const end = doc.data().periodEnd?.toDate?.();
  if (end && (!latestPeriodEnd || end > latestPeriodEnd)) {
    latestPeriodEnd = end;
  }
});

if (!existingPeriodsSnap.empty) {
  const latestPeriod = existingPeriodsSnap.docs[0].data();
  const latestEnd = latestPeriod.periodEnd?.toDate?.() || null;

  if (latestEnd && latestEnd > now) {
    latestPeriodEnd = latestEnd;
  }
}
